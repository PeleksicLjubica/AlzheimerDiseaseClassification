    ...
    for path in pathlib.Path(PATH_TO_TAGGED_FILES + "/" + path_to_tagged).iterdir():
        if path.is_file():
            ...
            if ".DS_Store" not in path.parts:
                ...
                if number_of_verbs+number_of_aux != 0:
                    ...
                    patient_log_file.write("ratio noun to verb: 0")
                    patient_log_file.write("ratio pronoun to verb: 0")
                if number_of_nouns+number_of_propn != 0:
                    patient_log_file.write(
                        "ratio pronoun to noun: " + str(number_of_pron / (number_of_nouns + number_of_propn)) + "\n")
                else:
                    patient_log_file.write("ratio pronoun to noun: 0")
